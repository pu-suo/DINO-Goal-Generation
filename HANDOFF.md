...
**Status (2026-06-17):** G0/G1/G2 ✅ · relational readout energy ✅ · dynamics rollout-trained (reliable ~K3.5) ✅ · Phase H+E + improvement sweep DONE — relational SUCCESS 0.64 → 0.79 (n=100, sim-grounded, 0.88× the 0.90 oracle ceiling) via hard-centroid readout decode; frozen WM, no goal image · **Phase L GATE = PASS — language command is load-bearing, the 0.79 anchor is NOT start-state exploitation.** See `dino_wm/docs/{HIERARCHICAL_PLANNER.md, PHASE_L_RESULTS.md}`. Canonical plan: `specs/PROJECT_DEFINITIVE_PLAN.md`. **Next milestone = Phase G (generalization); gated, NOT started.**

## Phase L — load-bearing / leak / decorrelation (2026-06-17) — see docs/PHASE_L_RESULTS.md
**GATE PASS.** No §0.3 boundary crossed (no re-encode/unfreeze/corpus-scale/metric-change; Phase G not started).
- **L1 leak probe (`lt_diag_leak.py`) = PASS:** goal tuple unrecoverable from z_start, held out by trajectory — at chance even from GROUND-TRUTH positions (unordered pair 0.044 vs 0.036; worst margin +0.021; probes overfit train to 1.000 ⇒ nothing to learn). Root cause: `_sample_objects` draws the pair uniform-random, independent of positions ⇒ goal decorrelated BY CONSTRUCTION. Also IS the §6 decorrelation verification.
- **L3 = anchor 0.79, no collapse:** the generator is decorrelated, so the eval set IS the decorrelated split; no separate correlated split exists to inflate the headline. 21/100 anchor failures all push toward the CORRECT anchor (distance-bound: far-pair overshoot + last-0.05u + red_moon), not goal-identity errors.
- **L2 swapped-command (n=100, same seeds):** correct **0.79** ≫ none **0.02**; final ‖A−B‖ correct 0.102u < none 0.213u(no motion) < wrong 0.247u(driven apart). Symmetric task ⇒ A↔B swap is a NO-OP (swap control 0.67 ≈ correct) so "wrong" = anchor-substitution; binary wrong 0.12 > none 0.02 is a lucky-anchor artifact (distance metric gives the right ordering). Per §4 L2 = tuple-conditioning; L1+L3 are the load-bearing evidence; NL grounding awaits the Phase-G VLM.
- **L4 multi-step forecast (DIAGNOSTIC, not gate):** per-subtask holds (~0.70) but end-to-end COLLAPSES — 2-step 0.30, **3-step 0.07 ≪ naive 0.36** (opposite of the pre-registered prediction). Mechanism: later subtasks disturb earlier-completed pairs (9/30 3-step eps reached all 3 but only 2 persisted). ⇒ **Phase-G compositional success needs interference-aware composition, not just a VLM decomposer.**
- **Infra bug fixed:** env-server socket inherited a 5s timeout from `create_connection` (governs waiting for the planner's next command); concurrent runs pushed CEM plans >5s → server died. Fix `sock.settimeout(1200)`. New code: `lt_loop.py --cmd {correct,none,wrong,swap}` + `--mode h3chain`; `lt_envserver.py no_terminate`. GPU FP nondeterminism ⇒ n=100 rates ~±0.02.
